...
class TestTokenManager:
    ...
    def setup_class(cls):
        """
        Set up for token manager tests.
        """
        import daemon  # pylint: disable=import-outside-toplevel

        cls.daemon = daemon

    def test_start_token_manager(self, credentials, config_dict, tokens_dict):
        """
    ...
    def test_daemonize_token_manager(self, config_dict):
        """
        Tests that token manager daemonizes token management and logs progress.
        """
        when(self.daemon.DaemonContext).__enter__().thenReturn(None)  # pylint: disable=unnecessary-dunder-call
        when(self.daemon.DaemonContext).__exit__(ANY, ANY, ANY).thenReturn(None)
    ...
    def test_daemonize_token_manager_uses_unix_recommended_log_dir(self, config_dict):
        """
    ...
    def test_daemonize_token_manager_uses_set_log_dir(self, config_dict):
        """
